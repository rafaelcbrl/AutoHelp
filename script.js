...
const telaIA = document.getElementById("telaIA");
...
const btnPerguntarIA = document.getElementById("btnPerguntarIA");
const btnVoltarIA = document.getElementById("btnVoltarIA");
...
function abrirTelaIA() {
  ...
  telaPerfil.classList.add("hidden");
  telaPerfilPublico.classList.add("hidden");
  telaIA.classList.remove("hidden");

  respostaIA.innerHTML = `
    <p>
      Descreva o problema do veículo para receber uma orientação inicial.
    </p>
  `;
}

btnVoltarIA.addEventListener("click", function () {
  telaIA.classList.add("hidden");
  telaPrincipal.classList.remove("hidden");
});
...
btnPerguntarIA.addEventListener("click", async function () {
  ...
  if (pergunta === "") {
    respostaIA.innerHTML = "<p>Digite uma dúvida sobre o veículo.</p>";
    return;
  }

  respostaIA.innerHTML = "<p>Consultando IA automotiva...</p>";

  try {

    const resposta = await fetch(`${API_URL}/ia/perguntar`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json"
      },
    ...
    if (!resposta.ok) {
      respostaIA.innerHTML = `
        <p>${resultado.mensagem || "Erro ao consultar a IA."}</p>
      `;
      return;
    }

    respostaIA.innerHTML = `
      <h3>Resposta da IA Automotiva</h3>
      <p>${resultado.resposta.replace(/\n/g, "<br>")}</p>
...
function responderIAAutomotiva(pergunta) {
  const texto = pergunta.toLowerCase();

  if (texto.includes("não liga") || texto.includes("nao liga") || texto.includes("não pega") || texto.includes("nao pega")) {
    return {
      titulo: "Possível problema de partida",
      categoria: "Partida / bateria",
      resposta: "As causas mais comuns são bateria descarregada, mau contato nos cabos, motor de partida, combustível insuficiente, chave/codificação ou falha elétrica. Verifique se o painel acende, se os faróis estão fracos e se faz algum clique ao virar a chave."
    };
  }

  if (texto.includes("bateria") || texto.includes("alternador")) {
    return {
      titulo: "Possível problema elétrico",
      categoria: "Bateria / alternador",
      resposta: "Se a bateria descarrega rápido, o problema pode estar na própria bateria, no alternador, em fuga de corrente ou em mau contato nos terminais. Se o carro apaga andando ou a luz da bateria acende, o alternador deve ser verificado."
    };
  }

  if (texto.includes("injeção") || texto.includes("injecao") || texto.includes("luz do motor")) {
    return {
      titulo: "Luz da injeção acesa",
      categoria: "Injeção eletrônica",
      resposta: "A luz da injeção pode indicar falha em sensores, bicos injetores, vela, bobina, combustível ruim, sonda lambda ou catalisador. Se o carro estiver falhando, perdendo força ou consumindo muito, o ideal é passar scanner automotivo."
    };
  }

  if (texto.includes("esquentando") || texto.includes("superaquec") || texto.includes("temperatura")) {
    return {
      titulo: "Risco de superaquecimento",
      categoria: "Arrefecimento",
      resposta: "Pode ser falta de água/aditivo, vazamento, ventoinha sem funcionar, válvula termostática travada, radiador obstruído ou bomba d’água. Não continue rodando com o motor superaquecendo, pois pode queimar junta do cabeçote."
    };
  }

  if (texto.includes("freio") || texto.includes("freando") || texto.includes("pastilha")) {
    return {
      titulo: "Possível problema nos freios",
      categoria: "Freios",
      resposta: "Ruído ao frear pode indicar pastilhas gastas, disco irregular ou sujeira. Pedal baixo ou borrachudo pode indicar ar no sistema, fluido velho ou vazamento. Se o freio falhar, não dirija o veículo."
    };
  }

  if (texto.includes("óleo") || texto.includes("oleo") || texto.includes("lubrificante")) {
    return {
      titulo: "Orientação sobre óleo",
      categoria: "Óleo / lubrificação",
      resposta: "Verifique o nível do óleo com o motor frio ou após alguns minutos desligado. Óleo baixo pode causar desgaste grave no motor. Use sempre a especificação recomendada no manual do veículo."
    };
  }

  if (texto.includes("pneu") || texto.includes("furou") || texto.includes("calibragem")) {
    return {
      titulo: "Problema relacionado aos pneus",
      categoria: "Pneus",
      resposta: "Pneu murcho, careca ou descalibrado prejudica frenagem, estabilidade e consumo. Se furou, pare em local seguro e use estepe ou solicite ajuda. Calibre conforme a recomendação do fabricante."
    };
  }

  if (texto.includes("barulho") || texto.includes("ruído") || texto.includes("ruido") || texto.includes("batendo")) {
    return {
      titulo: "Barulho anormal no veículo",
      categoria: "Ruídos",
      resposta: "Barulhos podem vir de suspensão, motor, correias, freios, rolamentos ou escapamento. Observe se o ruído aparece ao acelerar, frear, virar o volante ou passar em buracos. Isso ajuda a identificar a origem."
    };
  }

  if (texto.includes("fumaça") || texto.includes("fumaca") || texto.includes("escapamento")) {
    return {
      titulo: "Fumaça no escapamento",
      categoria: "Motor / escapamento",
      resposta: "Fumaça branca pode indicar água no motor ou condensação. Fumaça azul pode indicar queima de óleo. Fumaça preta pode indicar excesso de combustível. Se for constante, procure avaliação mecânica."
    };
  }

  if (texto.includes("câmbio") || texto.includes("cambio") || texto.includes("marcha") || texto.includes("embreagem")) {
    return {
      titulo: "Possível problema no câmbio ou embreagem",
      categoria: "Transmissão",
      resposta: "Dificuldade para engatar marchas pode ser embreagem gasta, cabo desregulado, fluido baixo ou problema no câmbio. Se a rotação sobe e o carro não desenvolve, a embreagem pode estar patinando."
    };
  }

  if (texto.includes("ar condicionado") || texto.includes("ar-condicionado") || texto.includes("gelar")) {
    return {
      titulo: "Problema no ar-condicionado",
      categoria: "Ar-condicionado",
      resposta: "Se o ar não gela, pode ser falta de gás, filtro de cabine sujo, compressor, vazamento ou problema elétrico. Se há mau cheiro, o sistema pode precisar de higienização."
    };
  }

  return {
    titulo: "Orientação automotiva geral",
    categoria: "Geral",
    resposta: "Pelo que você descreveu, o ideal é observar quando o problema acontece, quais luzes aparecem no painel, se há barulho, cheiro, fumaça, perda de força ou vazamento. Se o veículo apresentar risco de segurança, evite continuar rodando e solicite atendimento."
  };
}
